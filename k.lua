...
task.spawn(function()
            ...
            if SilentAim.enabled and SilentAim.currentTarget and typeof(dir) == "Vector3" then
                ...
                if c and c:FindFirstChild("Head") then
                    ...
                    if SilentAim.mode == "自动" then
                        shouldAim = true
                    elseif SilentAim.mode == "热键" then
                        if SilentAim.holdKey then
                    ...
                    -- 动态未命中逻辑
                    if SilentAim.dynamicMiss then
                        local hitChance = SilentAim.baseHitChance / 100
                        if math.random() > hitChance then
                            shouldAim = false -- 模拟未命中
                        end
                    end
                    ...
                    if shouldAim and distance < 12 then
                        ...
                        if not SilentAim.wallbang then
                            ...
                            local raycastResult = workspace:Raycast(orig, (SilentAim.currentTarget.Position - orig), raycastParams)
                            if raycastResult and raycastResult.Instance:IsDescendantOf(SilentAim.currentTarget) then
    ...
    -- 为 Rage 模块添加静默自瞄接口
    if rageBot then
        rageBot.SetSilentAim = function(self, value)
            SilentAim.enabled = value
        end
        
        rageBot.SetSilentAimMode = function(self, mode)
            SilentAim.mode = mode
        end
        
        rageBot.SetSilentAimHoldKey = function(self, key)
            SilentAim.holdKey = key
        end
        
        rageBot.SetSilentAimToggleKey = function(self, key)
            SilentAim.toggleKey = key
        end
        
        rageBot.SetWallbang = function(self, value)
            SilentAim.wallbang = value
        end
        
        rageBot.SetDynamicMiss = function(self, value)
            SilentAim.dynamicMiss = value
        end
        
        rageBot.SetBaseHitChance = function(self, value)
            SilentAim.baseHitChance = value
        end
        
        -- 目标更新函数（由Rage模块调用）
        rageBot.UpdateSilentAimTarget = function(self, target)
